  ...
  return (
    <>
      ...
      <main className="flex-1 pb-20">
        {/* Breadcrumb */}
        <Container>
          <nav className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-[0.12em] text-[--color-mp-text-soft] py-5">
            <Link href="/unidades" className="hover:text-[--color-mp-text]">
              Unidades
            </Link>
            <ChevronRight className="h-3 w-3" />
            <span>{unit.region}</span>
            <ChevronRight className="h-3 w-3" />
            <span className="text-[--color-mp-text]">{unit.name}</span>
          </nav>
        </Container>
        ...
        <Container>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold tracking-tight leading-[1.05] text-[--color-mp-ink]">
                {unit.name}
              </h1>
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm text-[--color-mp-text-soft]">
                ...
                <span className="flex items-center gap-1">
                  <Star className="h-3.5 w-3.5 fill-[--color-mp-warning] text-[--color-mp-warning]" />
                  <strong className="text-[--color-mp-text]">{unit.rating}</strong>
          ...
          <div className="grid lg:grid-cols-[1.45fr_1fr] gap-6 mt-8">
            <div className="space-y-6">
              ...
              {/* About */}
              <div className="rounded-2xl bg-white border border-[--color-mp-line] p-6 md:p-7">
                <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-[--color-mp-text-soft] mb-3">
                  Sobre essa unidade
                </div>
                <p className="text-[--color-mp-text] leading-relaxed">
                  {unit.description}
                </p>
              ...
              {/* FAQ */}
              <div className="rounded-2xl bg-white border border-[--color-mp-line] p-6 md:p-7">
                <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-[--color-mp-text-soft] mb-4">
                  Perguntas frequentes
                </div>
                <div className="divide-y divide-[--color-mp-line] -mx-1">
                  {FAQ.map((q) => (
                    <details key={q.q} className="group py-3.5 px-1 cursor-pointer">
                      <summary className="flex items-center justify-between font-medium list-none">
                        {q.q}
                        <ChevronRight className="h-4 w-4 text-[--color-mp-text-soft] group-open:rotate-90 transition-transform" />
                      </summary>
                      <p className="mt-2 text-sm text-[--color-mp-text-soft] leading-relaxed">
                        {q.a}
                      </p>
                    </details>
                  ))}
          ...
          <div className="mt-16">
            <div className="flex items-end justify-between mb-6">
              <h2 className="text-2xl md:text-3xl font-semibold tracking-tight text-[--color-mp-ink]">
                Outras unidades próximas
              </h2>
              <Link
                href="/unidades"
                className="text-sm font-medium text-[--color-mp-red] hover:underline"
              >
                Ver todas →
              </Link>
            </div>
              ...
              {UNITS.filter((u) => u.id !== unit.id)
                ...
                .map((u) => (
                  <Link
                    ...
                    className="block rounded-2xl border border-[--color-mp-line] bg-white p-5 hover:border-[--color-mp-text] transition-colors group"
                  >
...
function InfoTile({
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-[--color-mp-cream]/50">
...
function OccupancyCard({
  ...
  const tier =
    ocupacao < 50
      ? { label: "Disponível", color: "bg-[--color-mp-success]" }
      : ocupacao < 80
      ? { label: "Movimentado", color: "bg-[--color-mp-warning]" }
      : { label: "Quase lotado", color: "bg-[--color-mp-red]" };

  return (
    <div className="rounded-2xl border border-[--color-mp-line] p-5 bg-white">
      <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-[--color-mp-text-soft] mb-3">
        Ocupação ao vivo
      </div>
      <div className="flex items-baseline justify-between mb-3">
        <div>
          ...
          <span className="text-sm text-[--color-mp-text-soft] ml-1">
            de {total} vagas
          </span>
        </div>
        <span className="text-sm font-medium">{tier.label}</span>
      </div>
      <div className="h-2 bg-[--color-mp-cream] rounded-full overflow-hidden">
        <div
          className={`h-full ${tier.color} transition-all`}
          style={{ width: `${ocupacao}%` }}
        />
      </div>
      <div className="mt-2 text-xs text-[--color-mp-text-soft]">
