      ...
      <Container>
        <div className="max-w-2xl mb-14">
          ...
          <h2 className="text-3xl md:text-5xl font-semibold tracking-tight leading-[1.05] text-[var(--color-mp-ink)]">
            ...
            <br />
            <span className="text-[var(--color-mp-text-muted)]">em 3 passos.</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-px bg-[var(--color-mp-line)] rounded-2xl overflow-hidden border border-[var(--color-mp-line)]">
          {STEPS.map((s) => {
            ...
            return (
              <div
                key={s.n}
                className="bg-white p-8 md:p-10 group hover:bg-mp-cream-50 transition-colors"
              >
                <div className="flex items-start justify-between mb-8">
                  <span className="font-mono text-[11px] tracking-[0.18em] text-[var(--color-mp-red)]">
                    {s.n}
                  </span>
                  <div className="h-11 w-11 rounded-full bg-[var(--color-mp-cream)] flex items-center justify-center group-hover:bg-white transition-colors">
                    <Icon className="h-5 w-5 text-[var(--color-mp-ink)]" />
                  </div>
                </div>
                <h3 className="text-xl md:text-2xl font-semibold leading-tight text-[var(--color-mp-ink)] mb-3">
                  {s.title}
                </h3>
                <p className="text-sm md:text-base text-[var(--color-mp-text-soft)] leading-relaxed">
                  {s.desc}
                </p>
              </div>
            );
